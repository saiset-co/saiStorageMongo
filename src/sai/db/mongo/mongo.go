...
func StartMongod() {
	...
	if runtime.GOOS == "darwin" || runtime.GOOS == "linux" {
		...
		startMongoCmd := exec.Command("mongod --bind_ip 0.0.0.0")
		startMongoCmd.Start()
...
func FindOne(collectionName string, selector map[string]interface{}, result *interface{}) *common.Error {
	...
	err := collection.FindOne(context.TODO(), selector).Decode(&result)
...
func Find(collectionName string, selector map[string]interface{}, inputOptions interface{}, result *[]interface{}) *common.Error {
	...
	//fmt.Println("theOptionsLimit",theOptionsLimit["limit"])
	//fmt.Println("theOptionsSkip",theOptionsSkip["skip"])
